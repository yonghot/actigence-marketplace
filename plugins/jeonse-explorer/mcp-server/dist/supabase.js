 ...
 * 사기죄 양형기준 조회
 */
export async function getSentencingGuidelines(crimeType, typeNumber) {
    ...
    // Fallback: 로컬 JSON
    const guidelines = loadLocalData('sentencing-guidelines.json');
    return guidelines.guidelines.find((g) => g.crime_type === crimeType && g.type_number === typeNumber);
}
/**
 * 전세사기특별법 조문 조회
 */
export async function getSpecialActArticles(category) {
    if (client && !fallbackMode) {
    ...
    const data = loadLocalData('special-act.json');
    if (category) {
...
export async function getMolitStatistics() {
    ...
    const data = loadLocalData('molit-statistics.json');
...
export async function getPublicCases(filter) {
    ...
    const data = loadLocalData('sample-cases.json');
    return data.cases.filter((c) => c.fraud_type === filter.crime_type &&
