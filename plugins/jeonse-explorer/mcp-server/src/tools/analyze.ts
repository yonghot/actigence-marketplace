...
import {
  calculateEligibilityDistribution,
  calculateRecoveryDistribution,
  calculateSentencingDistribution,
...
export const analyzeInputSchema = z.object({
  deposit_range: z.enum(['under_50m', '50m_to_100m', '100m_to_300m', '300m_to_500m', 'over_500m']).describe(
    '보증금 액수 구간 (5천만원 이하/5천~1억/1억~3억/3억~5억/5억 초과)'
  ),
  priority_right: z.enum(['full_protection', 'registration_only', 'lease_registration', 'jeonse_right', 'none']).describe(
    '우선변제권 확보 여부 (전입+확정일자/전입만/임차권등기/전세권/없음)'
  ),
  landlord_property_count: z.enum(['one', 'two_to_ten', 'ten_to_hundred', 'over_hundred', 'unknown']).describe(
    '임대인의 임차주택 보유 수 (1채/2~10채/10~100채/100채+/모름)'
  ),
  auction_status: z.enum(['in_progress', 'decided', 'applied', 'not_started']).describe(
    '경매·공매 진행 단계'
  ),
  investigation_status: z.enum(['indicted', 'investigating', 'complaint_filed', 'no_complaint', 'unknown']).describe(
    '임대인 수사 단계'
  ),
  victim_count: z.enum(['self_only', 'two_to_ten', 'ten_to_hundred', 'over_hundred', 'unknown']).describe(
    '확인된 다른 피해자 수'
  ),
  region: z.enum([
...
export async function analyzeJeonseFraudDistribution(input: AnalyzeInput) {
  // 3개 분포를 병렬 계산
...
function buildUIResourceUri(input: AnalyzeInput, distributions: any): string {
