 ...
 */
export const PII_PATTERNS: Array<{ name: string; pattern: RegExp; severity: 'critical' | 'high' | 'medium' }> = [
  // 주민번호 (13자리, 6-7 형태)
  {
...
export interface PIIDetectionResult {
  detected: boolean;
  patterns: Array<{ name: string; matches: string[]; severity: string }>;
...
export function detectPII(input: string): PIIDetectionResult {
  ...
  const severityRank: Record<string, number> = { none: 0, medium: 1, high: 2, critical: 3 };
 ...
 */
export function sanitizeInput(args: Record<string, unknown>): Record<string, unknown> {
