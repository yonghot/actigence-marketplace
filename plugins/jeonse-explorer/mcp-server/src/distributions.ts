 ...
 */
export async function calculateEligibilityDistribution(input: AnalysisInput) {
  ...
  const r4 = scoreRequirement4_BadFaithIntent(
    input.investigation_status,
    input.landlord_property_count
  );
...
function scoreRequirement1_PriorityRight(p: PriorityRight): number {
  switch (p) {
...
function scoreRequirement2_DepositLimit(d: DepositRange, region: Region): number {
  ...
  const depositMidpoint: Record<DepositRange, number> = {
  ...
  if (midpoint <= limit) return 1.0;
  if (midpoint <= limit * 1.2) return 0.5;
  return 0.1;
}

function scoreRequirement3_ObjectiveLoss(a: AuctionStatus, v: VictimCount): number {
...
function scoreRequirement4_BadFaithIntent(i: InvestigationStatus, l: LandlordPropertyCount): number {
 ...
 */
export async function calculateRecoveryDistribution(input: AnalysisInput) {
  const buckets = [
 ...
 */
export async function calculateSentencingDistribution(input: AnalysisInput) {
...
function classifyFraudType(input: AnalysisInput): 'general' | 'organized' {
...
function estimateAggregateDamage(input: AnalysisInput): number {
  const depositMidpoint: Record<DepositRange, number> = {
    'under_50m': 30_000_000,
    '50m_to_100m': 75_000_000,
    '100m_to_300m': 200_000_000,
  ...
  const victimMultiplier: Record<VictimCount, number> = {
    'self_only': 1,
    'two_to_ten': 5,
    'ten_to_hundred': 50,
    'over_hundred': 200,
...
function mapToSentencingType(damage: number): number {
  if (damage < 100_000_000) return 1;        // 1억 미만
  if (damage < 500_000_000) return 2;        // 1억~5억
  if (damage < 5_000_000_000) return 3;      // 5억~50억
  if (damage < 30_000_000_000) return 4;     // 50억~300억
  return 5;                                   // 300억+
}

function getDamageRangeMin(type: number): number {
  return [0, 0, 100_000_000, 500_000_000, 5_000_000_000, 30_000_000_000][type] ?? 0;
}

function getDamageRangeMax(type: number): number {
...
function formatDamageRange(type: number): string {
  return ['', '1억 미만', '1억~5억', '5억~50억 (특경법)', '50억~300억 (특경법)', '300억 이상 (특경법)'][type] ?? '';
}

function fillBucketsFromGuideline(buckets: any[], guideline: any, fraudType: 'general' | 'organized') {
  // 양형기준 기반 추정 분포 (보수적)
  if (!guideline) {
    // 데이터 없음 → 균등 분포
...
function normalizeBuckets(buckets: Array<{ percentage: number }>): void {
...
function calculateMedian(buckets: Array<{ percentage: number; range: string }>): number {
  let cumulative = 0;
  for (const b of buckets) {
...
function calculateMedianSentence(buckets: any[]): number {
  let cumulative = 0;
  for (const b of buckets) {
    cumulative += b.percentage;
    if (cumulative >= 50) {
      if (b.range === '집행유예') return 0;
